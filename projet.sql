...
CREATE SEQUENCE Seq_ListeCourse_numListe START WITH 1 INCREMENT BY 1 NOCYCLE;
...
CREATE OR REPLACE TRIGGER Produit_numProduit
  ...
  FOR EACH ROW 
	...
	END;
CREATE OR REPLACE TRIGGER ListeCourse_numListe
	BEFORE INSERT ON ListeCourse 
  FOR EACH ROW 
	WHEN (NEW.numListe IS NULL) 
	BEGIN
		 select Seq_ListeCourse_numListe.NEXTVAL INTO :NEW.numListe from DUAL; 
	END;
